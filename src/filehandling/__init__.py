...
def _format_filetypes(filetypes):
    if not filetypes:
        return ""
...
def get_filename(
        ...
        parent=None):
    """
    Opens a QFileDialog to return a filename to open
    ...
    parent : widget
        If using with PyQt6 gui provide the parent so focus is returned.
...
def create_filename(
        ...
        parent=None):
    """
    Opens a QFileDialog to save a file.
...
def create_directory(
    ...
    """
    Opens a QFileDialog to create a directory
...
def smart_number_sort(filenames):
    """Assumes the number is at the end of the filename. Designed so that 1,2,3 then 01,02,03 then 001,002,003."""
    filename_sort=[]
...
def get_filenames(initialdir='/', title='Choose files', parent=None):
    _ensure_app()
